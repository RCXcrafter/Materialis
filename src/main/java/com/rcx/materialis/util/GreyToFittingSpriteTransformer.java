...
public class GreyToFittingSpriteTransformer implements ISpriteTransformer {
	...
	public GreyToFittingSpriteTransformer(List<SpriteMapping> sprites) {
	...
	private int getNewColor(int color, int x, int y, int width, int height) {
		...
		int newColor = getSpriteRange(grey).getColor(x, y, width, height, grey);
	...
	public void transform(NativeImage image) {
		for (int x = 0; x < image.getWidth(); x++) {
			for (int y = 0; y < image.getHeight(); y++) {
				image.setPixelRGBA(x, y, getNewColor(image.getPixelRGBA(x, y), x, y, image.getWidth(), image.getHeight()));
			}
		}
	}
	...
	public static class Builder {
		...
		public GreyToFittingSpriteTransformer build() {
			...
			return new GreyToFittingSpriteTransformer(list);
		}
	}
	...
	private static class SpriteMapping {
		...
		public int getColor(int x, int y, int width, int height) {
			if (path != null) {
				NativeImage image = getImage();
				if (image != null) {
					int spriteColor = image.getPixelRGBA((x * image.getWidth()) / width, (y * image.getHeight()) / height);
	...
	private record SpriteRange(@Nullable SpriteMapping before, @Nullable SpriteMapping after) {
		 ...
		 */
		public int getColor(int x, int y, int width, int height, int grey) {
			// after only
			if (before == null) {
				assert after != null;
				return after.getColor(x, y, width, height);
			}
			if (after == null || before == after) {
				return before.getColor(x, y, width, height);
			}
			return GreyToColorMapping.interpolateColors(before.getColor(x, y, width, height), before.getGrey(), after.getColor(x, y, width, height), after.getGrey(), grey);
		}
	}
