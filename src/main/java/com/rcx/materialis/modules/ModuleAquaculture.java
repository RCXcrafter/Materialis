...
public class ModuleAquaculture implements IModule {
	...
	public void registerItems(Register<Item> event) {
		if (!MaterialisConfig.blacklist.isMaterialBlacklisted("neptunium")) {
			OreDictionary.registerOre("ingotNeptunium", new ItemStack(AquacultureItems.metaLootItem, 1, 1));
		}
	}

	@Override
	public void init(FMLInitializationEvent event) {
		TinkerMaterials.bone.addItem(new ItemStack(AquacultureItems.fish, 1, 38), 1, Material.VALUE_Ingot);

		if (!MaterialisConfig.blacklist.isMaterialBlacklisted("neptunium")) {
			neptunium.addCommonItems("Neptunium");
			neptunium.setRepresentativeItem(new ItemStack(AquacultureItems.metaLootItem, 1, 1));
