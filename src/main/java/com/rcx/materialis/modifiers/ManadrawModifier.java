...
public class ManadrawModifier extends Modifier implements ConditionalStatModifierHook, ProjectileLaunchModifierHook {
	...
	public float modifyStat(IToolStackView tool, ModifierEntry modifier, LivingEntity living, FloatToolStat stat, float baseValue, float multiplier) {
		if (enabled && stat == ToolStats.DRAW_SPEED && !tool.isBroken() && living instanceof Player player) {
	...
	public void addInformation(IToolStackView tool, int level, Player player, List<Component> tooltip, TooltipKey key, TooltipFlag flag) {
		addStatTooltip(tool, ToolStats.DRAW_SPEED, TinkerTags.Items.RANGED, 0.15f * level, tooltip);
	}
}